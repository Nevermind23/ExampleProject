@extends('layouts.main', ['title' => 'Conversations Page'])
                    ...
                    @foreach(auth()->user()->conversations()->with('messages')->get() as $conversationItem)
                        <div class="chat_list {{$conversationItem->id == $conversation->id ? 'active_chat' : ''}}"
                            ...
                            <div class="chat_people">
                                <div class="chat_img"><img src="{{$conversationItem->secondUser->avatar}}"
                                                           alt="{{$conversationItem->secondUser->name}}"></div>
                                <div class="chat_ib">
                                    <h5>{{$conversationItem->secondUser->name}} <span
                                                class="chat_date">{{$conversationItem->updated_at->diffForHumans()}}</span>
                                    </h5>
                                    @if($conversationItem->messages()->exists())
                                        <p>{{\Illuminate\Support\Str::limit($conversationItem->messages()->orderByDesc('updated_at')->first()->text, 150, '...')}}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
